...
function UpdateInfo()
    ...
    local max = RING_CONTROLLER_CLIENT.context.GetMaxBuyable()
    SPEED_INCREASE_AMOUNT_DISPLAY.text = "[+"..UTILS.format(RING_CONTROLLER_CLIENT.context.BASE_SPEED * max, 3).."]"
    COST_DISPLAY.text = UTILS.format(-RING_CONTROLLER_CLIENT.context.GetCost(max), 0).." Points"
end

local function OnInteracted(whichTrigger, other)
    RING_CONTROLLER_CLIENT.context.BuyMax()
    UpdateInfo()
    RING_MANAGER_CLIENT.context.UpdateFillPerSecond()
    SPEED_INCREASE_AMOUNT_DISPLAY.text = "[+"..UTILS.format(0).."]"
    COST_DISPLAY.text = RING_CONTROLLER_CLIENT.context.level < RING_CONTROLLER_CLIENT.context.GetMaxLevel() and UTILS.format(0, 0).." Points" or "MAX LEVEL"
end
